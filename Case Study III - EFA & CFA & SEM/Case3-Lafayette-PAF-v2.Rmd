...
f_communality <- function(PAF){
  
    communality <- data.table("Item"=names(PAF$communality), 
                             "Communality"=as.numeric(PAF$communality))
    ...
    # Display table
    kbl <- communality |> 
              stable(caption=paste0("Communality for function \"",PAF$fn  ,"\" with rotation \"", PAF$rotation, "\" and ", PAF$factors, " factors")) |>
...
## f_communality_test {.unnumbered}

```{r}
# test to see if function calculates communality from loadings
f_communality_test <- function(PAF){
 
  communality_calc <- numeric()
  for (i in 1:nrow(PAF$loadings)){
    
    # test communality calculation
    variableloading = PAF$loadings[i,] # loadings 1st variable
    communality_calc[i] = sum(variableloading^2)
    names(communality_calc)[i] <- names(PAF$loadings[,1])[i]
    # print(paste0("Communality for ", names(PAF$loadings[,1])[i]," = ", communality_calc[i]))
    ...
    # Display table
    kbl <- communality |> 
              stable(caption=paste0("Test- Communality calculated from loadings for function \"",PAF$fn  ,"\" with rotation \"", PAF$rotation, "\" and ", PAF$factors, " factors")) |>
              row_spec(which(communality[,1]<.3), bold = T, color = "white", background = "red")
...
# f_communality_test(PAFn_norot[[k]])
# # print("----")
# f_communality(PAFn[[k]])
# f_communality_test(PAFn[[k]])
# # print("----")
# f_communality(PAFn_obl[[k]])
# f_communality_test(PAFn_obl[[k]])
```

```{r, echo=F}
# k=1
# f_communality(PCAn_norot[[k]])
# f_communality_test(PCAn_norot[[k]])
# # print("----")
# f_communality(PCAn[[k]])
# f_communality_test(PCAn[[k]])
# # print("----")
# f_communality(PCAn_obl[[k]])
# f_communality_test(PCAn_obl[[k]])
```
...
## f_eigenvalue_test {.unnumbered}

```{r}
f_eigenvalue_test <- function(PAF){

  Eigenvalue <- numeric()
  for (i in 1:ncol(PAF$loadings)){
  
    # test eigenvalue calculation
    factorloadings = PAF$loadings[,i] # loadings 1st factor (default is nfactors = 1)
    Eigenvalue[i] = sum(factorloadings^2)
    # print(paste0("Eigenvalue factor ",names(PAF$loadings[1,])[i] ," = ", Eigenvalue))
...
```{r}
f_kaiser <- function(PAF){

  #kaiser criterion (retain factors with eigenvalues >1)
  PAF_kaiser_nb <- length(which(PAF$e.values > 1))
  print(paste0("Kaiser number = ",PAF_kaiser_nb))
...
f_loadings <- function(PAF){
 
      # print(PAF$loadings, cutoff=0.3)
      # print(print_html(model_parameters(PAF, loadings=T, threshold = 0.3, summary=T))) 
      kbl <- model_parameters(PAF, loadings=T, threshold = 0.3, summary=T) |>
        stable()
      kbl 
}
```
...
```{r}
f_scree <- function(PAF){
  
  #display Scree-plot (retain factors before elbow)
  plot(PAF$e.values,xlab="Factor Number",
       ylab="Eigenvalue",
       main="Scree plot",
       cex.lab=1.2,
       cex.axis=1.2,
...
## f_totalvar_PAF {.unnumbered}

```{r}
f_totalvar_PAF <- function(PAF){
  
  #calculate total variance (does not change if number of factors change)
  PAF_EigenValue <- PAF$e.values
  PAF_Variance <- PAF_EigenValue / length(PAF$e.values) * 100 # ncol(data) = sum(PAF1$e.values) = length(PAF1$e.values)
  PAF_SumVariance <- cumsum(PAF_EigenValue / length(PAF$e.values))
  PAF_Total_Variance_Explained <- cbind("Factor number"=
                                            seq(1, length.out = length(PAF_EigenValue[PAF_EigenValue>0])),
                                            EigenValue = PAF_EigenValue[PAF_EigenValue>0],
                                            Variance = PAF_Variance[PAF_EigenValue>0],
                                            Total_Variance = PAF_SumVariance[PAF_EigenValue>0])
  #display table
  kbl <- PAF_Total_Variance_Explained |> 
    stable(caption=paste0("Total Variance Explained for function \"",PAF$fn  ,"\" calculated with \"e.values\" argument for rotation \"", PAF$rotation, "\" and ", PAF$factors, " factors")) 
...
## f_totalvar_PCA {.unnumbered}

```{r}
f_totalvar_PCA <- function(PCA){
  
  #calculate total variance (does not change if number of factors change)
  PCA_EigenValue <- PCA$values
  PCA_Variance <- PCA_EigenValue / length(PCA$values) * 100 # ncol(data) = sum(PCA$values) = length(PCA$values)
  PCA_SumVariance <- cumsum(PCA_EigenValue / length(PCA$values))
  PCA_Total_Variance_Explained <- cbind("Factor number"=
                                            seq(1, length.out = length(PCA_EigenValue[PCA_EigenValue>0])),
                                            EigenValue = PCA_EigenValue[PCA_EigenValue>0],
                                            Variance = PCA_Variance[PCA_EigenValue>0],
                                            Total_Variance = PCA_SumVariance[PCA_EigenValue>0])
  #display table
  kbl <- PCA_Total_Variance_Explained |> 
    stable(caption=paste0("Total Variance Explained for function \"",PCA$fn  ,"\" calculated with \"values\" argument for rotation \"", PCA$rotation, "\" and ", PCA$factors, " factors")) 
...
## f_totalvar_test {.unnumbered}

```{r}
f_totalvar_test <- function(PAF){
  
  #calculate total variance (does not change if number of factors change)
  PAF_EigenValue <- f_eigenvalue_test(PAF)
  PAF_Variance <- PAF_EigenValue / length(PAF$values) * 100 # ncol(data) = sum(PAF$values) = length(PAF$values)
  PAF_SumVariance <- cumsum(PAF_EigenValue / length(PAF$values))
  PAF_Total_Variance_Explained <- cbind("Factor number"=
                                            seq(1, length.out = length(PAF_EigenValue[PAF_EigenValue>0])),
                                            EigenValue = PAF_EigenValue[PAF_EigenValue>0],
                                            Variance = PAF_Variance[PAF_EigenValue>0],
  ...
  #display table
  kbl <- PAF_Total_Variance_Explained |> 
    stable(caption=paste0("Test - Total Variance Explained for function \"",PAF$fn  ,"\" calculated with loadings for rotation \"", PAF$rotation, "\" and ", PAF$factors, " factors")) 
...
# 
# f_totalvar_PCA(PAFn_norot[[k]])
# f_totalvar_PAF(PAFn_norot[[k]])
# f_totalvar_test(PAFn_norot[[k]])
# # print("----")
# f_totalvar_PCA(PAFn[[k]])
# f_totalvar_PAF(PAFn[[k]])
# f_totalvar_test(PAFn[[k]])
# # print("----")
# f_totalvar_PCA(PAFn_obl[[k]])
# f_totalvar_PAF(PAFn_obl[[k]])
# f_totalvar_test(PAFn_obl[[k]])
```
...
# 
# f_totalvar_PCA(PCAn_norot[[k]])
# f_totalvar_test(PCAn_norot[[k]])
# # print("----")
# f_totalvar_PCA(PCAn[[k]])
# f_totalvar_test(PCAn[[k]])
# # print("----")
# f_totalvar_PCA(PCAn_obl[[k]])
# f_totalvar_test(PCAn_obl[[k]])
```
...
for (i in 1:length(nf)) {
    ...
    print(f_totalvar_PAF(PAFn[[i]]))
    
    cat("\n")
    cat("###### from loadings {.unnumbered}" ,"\n")
    print(f_totalvar_test(PAFn[[i]]))
...
for (i in 1:length(nf)) {
    ...
    cat("###### from loadings {.unnumbered}" ,"\n")
    print(f_communality_test(PAFn[[i]]))
...
for (i in 1:length(nf)) {
    ...
    cat("###### from e.values {.unnumbered}" ,"\n")
    print(f_totalvar_PAF(PAFn_obl[[i]]))
    
    cat("\n")
    cat("###### from loadings {.unnumbered}" ,"\n")
    print(f_totalvar_test(PAFn_obl[[i]]))
...
for (i in 1:length(nf)) {
    ...
    cat("###### from loadings {.unnumbered}" ,"\n")
    print(f_communality_test(PAFn_obl[[i]]))
