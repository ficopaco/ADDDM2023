...
```{r}
EFA_PC0 <- psych::principal(data_img_EFA, rotate="varimax", scores=TRUE)
...
There are `r EFA_PC0_kaiser_nb` factors satisfying this condition.
...
EFA_PC0_Variance <- EFA_PC0_EigenValue / ncol(data_img_EFA) * 100
EFA_PC0_SumVariance <- cumsum(EFA_PC0_EigenValue / ncol(data_img_EFA))
...
```{r}
# perform multiple PCAs one for each factor number in selection
EFA_PCn = list()

i=1
for (n in nf) {
  # EFA_PCn[[i]] <- n
  EFA_PCn[[i]] <- psych::principal(data_img_EFA, rotate="varimax", scores=TRUE, nfactors = n)
  i=i+1
}
names(EFA_PCn) <- nf
...
for (n in nf) {
  # EFA_PCn_obl[[i]] <- n
  EFA_PCn_obl[[i]] <- psych::principal(data_img_EFA, rotate="promax", scores=TRUE, nfactors = n)
  i=i+1
}
...
# best 7 factors
exclude=c("Im9","Im11","Im15")

# best 6 factors
# exclude=c("Im16","Im9", "Im19","Im15") #"Im15"
...
```{r}
EFA_PC0 <- psych::principal(data_img_EFA2, rotate="varimax", scores=TRUE)
# note: by default number of factors = 1 if it is not specified
```
...
#calculate total variance (does not change if number of factors change)
EFA_PC0_EigenValue <- EFA_PC0$values
EFA_PC0_Variance <- EFA_PC0_EigenValue / ncol(data_img_EFA2) * 100
EFA_PC0_SumVariance <- cumsum(EFA_PC0_EigenValue / ncol(data_img_EFA2))
EFA_PC0_Total_Variance_Explained <- cbind("Factor number"=
                                            seq(1, length.out=length(EFA_PC0_EigenValue[EFA_PC0_EigenValue>0])),
                                          EigenValue = EFA_PC0_EigenValue[EFA_PC0_EigenValue>0],
                                          Variance = EFA_PC0_Variance[EFA_PC0_EigenValue>0],
...
for (n in nf) {
  # EFA_PCn[[i]] <- n
  EFA_PCn[[i]] <- psych::principal(data_img_EFA2, rotate="varimax", scores=TRUE, nfactors = n)
  i=i+1
}
names(EFA_PCn) <- nf
...
for (i in 1:length(nf)) {
  ...
  cat("###### Number of factors =", nf[[i]], "{.unnumbered}" ,"\n")
    ...
    # test communality calculation
    variableloading = EFA_PCn[[i]]$loadings["Im6",] # loadings 1st variable
    communality = sum(variableloading^2)
...
for (n in nf) {
  # EFA_PCn_obl[[i]] <- n
  EFA_PCn_obl[[i]] <- psych::principal(data_img_EFA2, rotate="promax", scores=TRUE, nfactors = n)
  i=i+1
}
names(EFA_PCn_obl) <- nf
...
for (i in 1:length(nf)) {
  ...
  cat("###### Number of factors =", nf[[i]], "{.unnumbered}" ,"\n")
    ...
    # test communality calculation
    variableloading = EFA_PCn_obl[[i]]$loadings["Im6",] # loadings 1st variable
    communality = sum(variableloading^2)
    print(paste0("Communality for Im6 =", communality))
    cat("\n")
...
# CFA_fit_img <- cfa(CFA_model_img, data=data_img_EFA, missing="ML")
CFA_fit_img <- cfa(CFA_model_img, data=survey, missing="ML")
...
# numcol_scores = dim(EFA_PCn[[3]]$scores)[2]
# numcol_data_img_EFA
# numcol_scores
# 
# CFA_data = cbind(data_img_EFA2, EFA_PCn[[3]]$scores, survey_excl_img2["SAT_1"])
# CFA_data
...
```{r}
# path analysis
cfa_SAT_1 <- cfa(model_SAT_1, data=survey, missing="ML")
