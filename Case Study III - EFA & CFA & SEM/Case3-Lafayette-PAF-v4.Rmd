...
date: "`r Sys.Date()`"
abstract:
  For businessess, understanding their clients consumption behavior is of upmost importance to be able to establish a competitive advantage, offer relevant the goods and services and determine marketing strategies that maximize the company benefits. Therefore, in this paper, we identify the key drivers of brand equity for Galeries Lafayette based on a questionnaire mailed to 5000 customers and returned by 600 of them. This is achieved by performing exploratory factor analysis to determine the most appropriate grouping of factors. Confirmatory factor analysis is used to confirm these constructs are statistically affirmed. Finally, Structured Equation Models (SEM) are used to evaluate causal relationships between mediator variables such as Customer Satisfaction and Affective Commitment on the objective attributes of Repurchase Intention and Co-Creation Intention. 
output: 
  html_document: 
    theme: readable
    highlight: pygments
...
The following function was developed by the modelers to facilitate display of results. 
...
The following functions were developed by the analysts to facilitate calculation and iteration of procedures. 
...
The data consists of `r dim(survey)[1]` records. There are 45 variables corresponding to the answers provided by the responding customers. 

## Clean and handle missing data {.unnumbered}

Next, variables are removed from the data set as they will not be considered for the analysis. Items with value of `999` are transformed as `NA` to equate to the proper data type. 
...
## Qurstion labels {.unnumbered}

To have a better understanding of the type of questions asked, the labels for each question is imported and displayed. 
...
As so we can see that survey intends to collect information regarding various customer perceptions regarding the associations made with regard to their brand and store. There are 22 questions that are regarding what the brand represents to the customer and 20 that are grouped in 6 constructs with the purpose to identify clear attributes of the store. As a reminder, `TRUST`, `SATISFACTION EMPLOYEES` are not considered in this analysis. 
...
We begin by performing PFA to identify potential factors that might help in determining the main drivers of Customer Satisfaction and Affective Commitment. 
...
By removing the `NA` values, we end up with `r dim(data_img_EFA)[1]` records and `r dim(data_img_EFA)[2]` variables. 
...
The KMO is above 0.6 which indicates the data is well suited for factor analysis.
...
In this case, we end up with `r dim(data_img_EFA)[1]` records and `r dim(data_img_EFA)[2]` variables. 
