...
The low p values for the parameter associated to our predictor ($\log(number_of_solar_panels)$) indicates that this predictor has a statistically significant relationship with the response ($\log(manufacturing_cost)$)
...
For information purposes we have also included prediction interval curves and values
...
pred_int_pred <- predict(lm.out, newdata=data.frame(x=log(newx)), interval="prediction",
                         level = 0.95)
conf_int_pred <- predict(lm.out, newdata=data.frame(x=log(newx)), interval="confidence",
                         level = 0.95)
